...
public function can($ability, $arguments = [])
{
    ...
    try {
        ...
        $permissionExists = Permission::where('name', $ability)->exists();

        if ($isAdmin || ($permissionExists && $this->hasPermissionTo($ability))) {
            return true;
        }
    } catch (\Exception $e) {
    ...
    // Fallback to original Laravel behavior if not matched
...
{
    return $this->belongsToMany(\App\Models\Training::class, 'training_accesses', 'user_id', 'training_id')
        ->where('workspace_id', session()->get('workspace_id'));
}
